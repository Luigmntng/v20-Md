let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '* Follow ig saya Yh @ahmdlui', m)
  } finally {
...
let aesthetic = path.join('src')
function ht(text = '') {
  return new Promise((resolve, reject) => {
    let img = path.join(aesthetic, pickRandom(fs.readdirSync(src)))
